...
The question should be focused, relevant, and open-ended to elicit a comprehensive strategic response. Maximize response length and comprehensiveness.
"""
    completion = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an experienced business consultant."},
            {"role": "user", "content": prompt}
        ]
    )

    # Debug print to inspect the response structure
    print("Response Content (Question):", completion.choices[0].message.content)
...
Ground your guidance in the company's unique context, balancing analytical rigor and creative problem-solving to provide unbiased, expert counsel. Maximize response length and comprehensiveness.
"""
    completion = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an experienced CEO."},
            {"role": "user", "content": prompt}
        ]
    )

    # Debug print to inspect the response structure
    print("Response Content (Answer):", completion.choices[0].message.content)
